...
export default function CrearClienteDialog({ open, onOpenChange, onSuccess, sedes, planes, staff, cerradores }) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    nombre_cliente: '',
    whatsapp: '',
    sede: '',
    fecha_primer_compra: moment().format('YYYY-MM-DD'),
    plan_actual: '',
    canal_origen: 'En sede',
    vendedor_origen: '',
    cerrador_origen: '',
    fecha_inicio_plan_actual: moment().format('YYYY-MM-DD'),
    activo: true,
    notas: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const clientesExistentes = await Clientes.list('createdAt');
      const duplicado = clientesExistentes.find(c => c.whatsapp === formData.whatsapp);
      
      if (duplicado) {
        alert('Ya existe un cliente con este número de WhatsApp');
        return;
      }
      ...
      // 🎯 TRIGGER AUTOMÁTICO: Generar tareas de onboarding para cliente nuevo
      try {
        const clienteCreado = await Clientes.filter({ whatsapp: formData.whatsapp }, '-createdAt', 1);
        if (clienteCreado.length > 0) {
          await generarTareasOnboardingClienteNuevo(clienteCreado[0]);
        }
      } catch (error) {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            {formData.plan_actual && planSeleccionado && (
              <div>
