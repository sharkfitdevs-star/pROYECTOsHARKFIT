...
function buildMensaje(kpi, valor, clas) {
  ...
  const msgs = {
    tasa_agendamiento:     { title: `Escasez de leads - Tasa agendamiento: ${pct}%`,     description: `Nivel ${clas} (${pct}%). Umbral: Alto 30%+, Normal 20-29%, Bajo <20%.` },
    conversion_venta:      { title: `Baja conversion de venta: ${pct}%`,                 description: `Nivel ${clas} (${pct}%). Umbral: Alto 25%+, Normal 15-25%, Bajo <15%.` },
    tasa_asistencia:       { title: `Tasa de asistencia: ${pct}%`,                       description: `Nivel ${clas} (${pct}%). Umbral: Alto 70%+, Normal 60-70%, Bajo <60%.` },
    conversion_asistentes: { title: `Baja conversion de asistentes: ${pct}%`,            description: `Nivel ${clas} (${pct}%). Umbral: Alto 70%+, Normal 55-70%, Bajo <55%.` },
  };
...
async function calcularYGenerarAlertas(opts = {}) {
  ...
  const resumen = { periodo: { desde, hasta }, conteos: { totalLeads, totalAsistieron, totalCompraron }, kpis, alertasCreadas: [], alertasOmitidas: [] };
  const mesKey = `${desde.getFullYear()}-${String(desde.getMonth()+1).padStart(2,'0')}`;
  for (const [kpi, valor] of Object.entries(kpis)) {
    if (valor === null) { resumen.alertasOmitidas.push({ kpi, razon: 'Sin datos suficientes' }); continue; }
    const clas = clasificar(kpi, valor);
    if (clas !== 'bajo') { resumen.alertasOmitidas.push({ kpi, clas, valor, razon: 'KPI en nivel aceptable' }); continue; }
    const idAlert = `kpi_${kpi}_${mesKey}`;
    const yaExiste = await Alerta.findOne({ idAlert, status: { $in: ['pendiente','en_proceso'] } });
    if (yaExiste) { await Alerta.updateOne({ idAlert }, { $set: { 'alertData.customData.valorActual': valor, updatedAt: new Date() } }); resumen.alertasOmitidas.push({ kpi, razon: 'Alerta activa ya existe (actualizada)' }); continue; }
    const { title, description } = buildMensaje(kpi, valor, clas);
    await new Alerta({ idAlert, type: 'kpi_rendimiento', priority: prioridadDesdeClasificacion(clas), status: 'pendiente', title, description, automatic: true, source: 'kpi_engine', alertData: { customData: { kpi, valorActual: valor, clasificacion: clas, periodo: { desde, hasta }, conteos: resumen.conteos } }, expiresAt: new Date(hasta.getFullYear(), hasta.getMonth() + 2, 1) }).save();
    resumen.alertasCreadas.push({ kpi, valor, clas, idAlert });
  }
  return resumen;
}
